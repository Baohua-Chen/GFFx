...
use crate::{
    ...
    safe_mmap_readonly, write_gff_output,
};
...
fn extract_features(
    ...
    let process_feature = |fid: &String| -> Option<Vec<u8>> {
        let numeric_id = match fts_map.get(fid.as_str()) {
            Some(&id) => id,
            None => {
                eprintln!("Warning: Feature '{}' not found in .fts", fid);
                return None;
            }
        };

        let root_id = match resolve_root(numeric_id, prt_map) {
            Ok(id) => id,
            Err(err) => {
        ...
        match process_one(fid, fts_map, prt_map, gof_map, gff_buf, descendants_only) {
            Ok(slice) => Some(slice),
            Err(err) => {
                eprintln!("Warning: {} (feature '{}')", err, fid);
                None
    ...
    let use_parallel = threads > 1 && feature_ids.len() > 2;
...
pub fn run(args: &ExtractArgs) -> Result<()> {
    ...
    let gff_buf = &safe_mmap_readonly(gff_path)?[..];
...
/// Extract a single model or descendants
fn process_one<'a>(
    fid: &str,
    fts_map: &HashMap<&str, u32>,
    prt_map: &HashMap<u32, u32>,
