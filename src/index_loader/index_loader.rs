...
pub fn load_sqs<P: AsRef<Path>>(path: P) -> Result<Vec<String>> {
    let path = path.as_ref();
    let new_path = append_suffix(path, ".sqs");
    let file_raw = File::open(&new_path)
        .with_context(|| format!("Failed to open SQS file: {:?}", &new_path))?;
    let file = BufReader::new(file_raw);
    Ok(file.lines().collect::<Result<_, _>>()?)
}
